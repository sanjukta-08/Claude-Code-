    ...
    <PageShell>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-5">
          ...
          <Panel padded={false}>
            <div className="flex items-center justify-between px-5 md:px-6 pt-5 mb-3">
              <div className="flex items-center gap-2">
                <span className="font-mono text-[10px] tracking-wide3 text-orange">NEEDS YOUR ATTENTION</span>
              </div>
              <Link to="/admin/challenges" className="font-mono text-[10px] tracking-wide3 text-ink-ghost hover:text-orange transition flex items-center gap-1.5">
                ALL <IconArrowRight size={11} />
              </Link>
            </div>
            {attention.length === 0 && closed.length === 0 ? (
              <div className="p-5 md:p-6 pt-0">
                <div className="rounded-lg border border-line/70 bg-bg px-5 py-8 text-center">
                  <div className="font-sans text-[15px] text-ink mb-1">All clear.</div>
                  <div className="font-sans text-[12.5px] text-ink-dim">No challenges need attention right now.</div>
                </div>
              </div>
            ) : (
        ...
        <div className="space-y-5">
          <Panel padded={false}>
            <div className="flex items-center justify-between px-5 pt-5 mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-[10px] tracking-wide3 text-ink-dim">RECENT SUBMISSIONS</span>
              </div>
              <span className="font-mono text-[9px] tracking-wide3 text-ink-ghost">LIVE</span>
            </div>
            <div className="divide-y divide-line/70">
              {recentSubs.map((s) => {
                ...
                return (
                  <Link
                    ...
                    className="group flex items-center gap-3 px-5 py-3 hover:bg-orange/[0.04] transition-colors"
                  >
                    <Avatar name={c.name} size="sm" tone="gold" />
                    <div className="flex-1 min-w-0">
                      <div className="font-sans text-[13px] text-ink truncate group-hover:text-orange transition">
                        {c.name}
                      </div>
                    ...
                    <div className="text-right">
                      <div className="font-sans text-[14px] text-ink tabular">{s.scores.total}</div>
                      <div className="font-mono text-[9.5px] tracking-wide2 text-ink-ghost tabular">
          ...
          {/* Quick actions */}
          <Panel>
            <div className="font-mono text-[10px] tracking-wide3 text-orange mb-4">QUICK ACTIONS</div>
...
function AttentionRow({ c, kind, subs }) {
  return (
    <Link
      ...
      className="group flex items-center gap-4 px-5 md:px-6 py-4 hover:bg-orange/[0.04] transition-colors"
    >
      <Avatar logo={c.company.logo} size="md" tone="gold" />
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          <span className="font-sans text-[14px] text-ink truncate">{c.role}</span>
      ...
      <div className="hidden md:flex flex-col items-end gap-1">
        {c.status === 'closed' ? (
          <span className="font-mono text-[10px] tracking-wide3 text-orange">PICK TOP N →</span>
        ) : (
          <Countdown deadline={c.deadline} className="text-[12px]" />
        )}
      </div>
      <IconArrowRight size={14} className="text-ink-ghost group-hover:text-orange transition" />
    </Link>
  )
}
...
function LiveChallengeCard({ c, subs }) {
  return (
    <Link
      to={`/admin/challenges/${c.id}`}
      className="group block p-5 bg-canvas hover:bg-orange/[0.04] transition-colors"
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-center gap-2.5">
          <Avatar logo={c.company.logo} size="sm" tone="gold" />
          <div>
            <div className="font-sans text-[13.5px] text-ink leading-tight">{c.company.name}</div>
            <div className="font-mono text-[9.5px] tracking-wide2 text-ink-ghost">{c.id}</div>
          </div>
        </div>
      ...
      <div className="font-sans text-[15.5px] text-ink leading-snug mb-3 truncate">{c.role}</div>
...
function QuickAction({ to, icon, label, hint }) {
  return (
    <Link
      to={to}
      className="group flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-bg transition-colors"
    >
      <span className="h-8 w-8 rounded-md border border-line bg-noir/[0.03] flex items-center justify-center text-orange group-hover:border-orange/40 transition">
        {icon}
      </span>
      <div className="flex-1 min-w-0">
        <div className="font-sans text-[13px] text-ink">{label}</div>
        <div className="font-mono text-[9.5px] tracking-wide2 text-ink-ghost">{hint}</div>
      </div>
      <IconArrowRight size={12} className="text-ink-ghost group-hover:text-orange transition" />
    </Link>
  )
}

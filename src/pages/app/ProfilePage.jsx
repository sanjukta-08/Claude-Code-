...
export default function ProfilePage() {
  ...
  if (!candidate) {
    return <PageShell><div className="font-mono text-ink-ghost">Loading…</div></PageShell>
  }
  ...
  return (
    <PageShell>
      ...
      >
        <Panel padded={false} className="overflow-hidden bg-gradient-to-br from-orange/[0.04] to-transparent border-orange/20">
          ...
          <div className="p-7 md:p-9 grid grid-cols-1 md:grid-cols-[auto_1fr_auto] gap-6 md:gap-10 items-center">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-1 font-sans text-[13.5px] text-ink-dim">{candidate.currentRole}</div>
            ...
            <div className="text-left md:text-right">
              <div className="font-mono text-[9.5px] tracking-wide3 text-ink-ghost">AGGREGATE AIQ</div>
              <div className="font-sans font-bold tracking-tighter text-[56px] md:text-[72px] leading-none text-orange tabular">
                {aiq.total ? aiqValue : '—'}
              </div>
              <div className="font-mono text-[10px] tracking-wide2 text-ink-ghost mt-1">
      ...
      {aiq.dimensions && (
        <section className="mt-10">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <span className="font-mono text-[10px] tracking-wide3 text-orange">DIMENSION PROFILE · AVG ACROSS ALL CHALLENGES</span>
          </div>
      ...
      <section className="mt-10">
        <div className="flex items-center justify-between gap-3 mb-4">
          <div className="flex items-center gap-2.5">
            <span className="h-px w-5 bg-orange/60" />
            <span className="font-mono text-[10px] tracking-wide3 text-orange">CHALLENGE HISTORY</span>
          </div>
        ...
        {subs.length === 0 ? (
          ...
          <Panel padded={false}>
            <div className="grid grid-cols-[40px_1fr_80px_80px_24px] md:grid-cols-[40px_1fr_140px_80px_80px_24px] gap-3 px-5 py-3 border-b border-line font-mono text-[9px] tracking-wide3 text-ink-ghost">
              <div></div>
              <div>ROLE</div>
            ...
            {subs.map((s, i) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0">
                    <div className="font-sans text-[13.5px] text-ink truncate group-hover:text-orange transition">{ch.role}</div>
                    <div className="font-mono text-[10px] tracking-wide2 text-ink-ghost truncate">{ch.company.name} · {ch.id}</div>
                  </div>
                  <div className="hidden md:block font-mono text-[10.5px] tracking-wide2 text-ink-ghost tabular">{formatRelative(s.submittedAt)}</div>
                  <div className="text-right font-sans text-[15px] text-ink tabular">{s.scores.total}</div>
                  <div className="text-right font-mono text-[10.5px] tracking-wide2 text-ink-dim tabular">
...
function KpiInline({ label, value, accent, mono }) {
  return (
    <Panel className="!py-4">
      ...
      <div className={`mt-2 leading-none ${mono ? 'font-mono text-[14px]' : 'font-sans font-bold tracking-tighter text-[28px] tabular'} ${accent ? 'text-orange' : 'text-ink'}`}>
        {value || '—'}
      </div>
    </Panel>
  )
